        ...
        <div class="relative z-10">
            <div class="flex items-center space-x-4 mb-4">
                <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-sm">
                    <i class="ri-coupon-edit-line text-2xl text-white"></i>
                </div>
                <div>
                    <h1 class="text-2xl lg:text-3xl font-bold text-white mb-1">Modifier le Coupon</h1>
                    <p class="text-purple-100">Modifiez les détails du coupon {{ $coupon->code }}</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="form-input w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-gray-700 dark:text-white"
                                  placeholder="Description optionnelle du coupon">{{ old('description', $coupon->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <div class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500" id="value-suffix">{{ $coupon->type === 'percentage' ? '%' : '€' }}</div>
                            </div>
                            @error('value')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="space-y-2 max-h-48 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg p-3">
                        ...
                        <label class="flex items-center space-x-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg">
                            <input type="checkbox" name="applicable_ebooks[]" value="{{ $ebook->id }}"
                                   {{ in_array($ebook->id, old('applicable_ebooks', $coupon->applicable_ebooks ?? [])) ? 'checked' : '' }}
                    ...
                    <div class="space-y-2 max-h-48 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg p-3">
                        ...
                        <label class="flex items-center space-x-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg">
                            <input type="checkbox" name="applicable_categories[]" value="{{ $category->id }}"
                                   {{ in_array($category->id, old('applicable_categories', $coupon->applicable_categories ?? [])) ? 'checked' : '' }}
                                   class="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary dark:focus:ring-primary dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600">
